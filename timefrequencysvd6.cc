...
void load_all_data(vector<Feature> &v1, vector<Feature> &v2, vector<Feature> &v3, vector<Feature> &v4, vector<Feature> &v5, vector<float> &Tu, vector<vector<int>> &Fut){
...
public:
    TimeFrequencySVD(vector<float> &Tuu, vector<vector<int>> &Futt) : Tu(Tuu), Fut(Futt) {
...
int main(){
	...
	for(int i = 0; i < 200; i++){
		...
		if(testscore  < mintestscore){
			...
			model.predict(qual, "output_timefrequencysvd6("+ to_string(i) +").dta");
		}
	}
}
