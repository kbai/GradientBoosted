...
#define NUSER 458293
#define NMOVIE 17770
#define NF 20 // number of latent factors
using namespace std;
...
/*
#iteration = 80
rmse on quiz set = 0.89524
output output_svdpp(f=50)_predict.dta generated.
*/
...
public:
    ...
    void predict_features(vector<Feature> &set, string s){
        int N = set.size();
        ofstream file(s);
        for(Feature &data : set){
            int u = data.u;
...
int main(){
	...
	model.predict_features(qual, "output_svdpp20features_predict.dta");
}
