...
/*
#iteration = 15
rmse on valid set = 0.792634
rmse on probe set = 0.898258
output output_timesvd.dta generated.
*/
...
private:
	...
	vector<vector<double>> pu = vector<vector<double>>(NUSER, vector<double>(NF,0));
	vector<vector<double>> alphau = vector<vector<double>>(NUSER, vector<double>(NF,0));
	vector<vector<double>> qm = vector<vector<double>>(NMOVIE, vector<double>(NF,0));	
	...
	double eta_pu = 0.008, lambda_pu = 0.015;
	double eta_alphau = 1E-5,  lambda_alphau = 50;
	double eta_qm = 0.008, lambda_qm = 0.015;

public:
    TimeSVD(vector<double> &Tuu) : Tu(Tuu) {
        uniform_real_distribution<double> u(0,1);
        for(vector<double> &v : pu) for(double &x : v) x = u(e) / NF;
        for(vector<double> &v : qm) for(double &x : v) x = u(e) / NF;
    }
...
int main(){
	...
	for(int i = 0; i < 200; i++){
		...
		if(testscore  < mintestscore){
			...
			model.predict(qual, "output_timesvd.dta");
		}
	}
}
