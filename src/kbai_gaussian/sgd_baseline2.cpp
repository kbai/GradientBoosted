...
#include <cstdlib>
using namespace std;
int sgd(vector<int> & uuser, vector<int> & umovie,float &mean, vector<float> &bu, vector<float> &bm,vector<float>& bs, ifstream &ofile,float lambda, float alpha ,float lr)
{
	int iu;
	int im;
	int it;
	...
	iu--;
	im--;
	error = -(mean + bu[iu] + bm[im]*(1+bs[iu])) + rate ;
...
double compute_RMSE(vector<vector<int>> &testset,vector<float> &bu,vector<float> &bm,vector<float> &bs, float mean)
{
	int im,iu,rate;
	int n = testset.size();
	double rmse = 0.0;
...
//
//		cout << rmse << endl;
		rmse +=pow( (mean + bu[iu] + bm[im]*(1+bs[iu]) -rate),2.0)/n;
		if(rmse> 1e6)
		{
...
double compute_QUAL(vector<vector<int>> &testset,vector<float> &bu,vector<float> &bm,vector<float> &bs, float mean)
{
	int im,iu,rate;
	int n = testset.size();
	double rmse = 0.0;
...
//		cout << rmse << endl;
		...
		qual << mean + bu[iu] + bm[im]*(1+bs[iu]) << endl;
//		rmse = sqrt(rmse);
//		cout << testset.size() << endl;
//		exit(1);
	}
...
int main()
{
	...
	for(int j = 0; j < 100; j++)	
	{
		...
		sgd(uuser,umovie,mean,bu,bm,bs,infile,lbd,alpha,0.01);
		cout << compute_RMSE(testset,bu,bm,bs, mean)<<endl;
		cout << compute_RMSE(testset2,bu,bm,bs, mean) << endl;
//		cout << mean << endl;
	}

	compute_QUAL(testset3, bu, bm,bs, mean);
