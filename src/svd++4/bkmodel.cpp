...
//	pfm(NMOVIE,vector<vector<float>>(8,vector<float>(NLAT,0)))
	...
	pmf(NLAT,vector<float>(NMOVIE*8,0)),
	y(NLAT,vector<float>(NMOVIE,0)),
	sumy(NLAT,vector<float>(NUSER,0)),
	sumyg(NLAT,vector<float>(NUSER,0))
{
...
float bkmodel::g(int iu,int itu, int im, int ita, int ife,float tt)
{
	float gv = 0.0;
	int it = (int)(ita/75);
	vector<float> tmp(NLAT,0);
	vector<float> tmp2(NLAT,0);
	for(int i = 0 ; i < NLAT;i++) tmp[i] = pu[iu][i]+pu1[iu][i]*(u(tt)+htu[iu])+ptu[i/10][itu]+sumy[i][iu];//+ptu[iu*30+it][i];
	for(int i = 0 ; i < NLAT;i++) tmp2[i]= pm[im][i] + pmf[i][im*8+ife];//+ptu[iu*30+it][i];

	gv = mean + bu[iu]+ but[itu] + au[iu]*u(tt) + dotprod(tmp,tmp2) +  (bm[im]+ btm[im][it])*(btu[iu]+cut[itu])// songming model.
	   	+ bt[it] + bta[ita] + bf[ife] + bfm[im][ife];
	return gv;
}
...
void bkmodel::update_param_sgd(feature &a)
{
	...
	float error = -g(iu,itu,im,ita,ife,tt) + rate ;
