...
//cout <<"\t"<<imovie.size() <<"\t" <<ph_mean[i] <<"\t" <<input[5*jr + ns] <<" "<< nh_means[i]<<"\t"<< nv_means[5*jr+ns] << endl;
    ...
    hbias[i] += vhbias[i]; 
#else
	hbias[i] += 0.1 * lr * (ph_mean[i] - nh_means[i] - 0.001 * hbias[i]); 
#endif	
  }
  //cout << "cd2" << endl;
...
#else
		vbias[5*im + ns]        +=  4.0 * lr * (bias - 0.001 *vbias[5*im + ns]);
		bu[5*iu + ns]			+=  4.0 * 0.25*lr*(bias - 0.001* bu[5*iu+ns]);
		but[5*iut+ns]			+=  4.0 * 0.80*lr*(bias - 0.001* but[5*iut+ns]);
		bif[5*((30*im)+ife)+ns] +=  4.0 * 0.02*lr*(bias - 0.001* bif[5*((30*im)+ife)+ns]);
#endif
	  }
  }
  mtx.unlock();
...
//    	 cout <<"error" << compute_loss(trainuid,testuid,trainset,train_index,testset,test_index,rbm) << endl;
	...
	for(int iepo = 0 ; iepo < 50; iepo++)
	{
		...
		//cout <<"4545" <<endl;
		 ...
		 if(writedata[0].joinable()) writedata[0].join();
		 if(writedata[1].joinable()) writedata[1].join();
			 writedata[0] = std::thread(compute_loss,std::ref(trainuid),std::ref(trainuid),std::ref(trainset),std::ref(train_index),std::ref(trainset),std::ref(train_index),std::ref(rbm2),"residue1234.dta");
			 writedata[0].join();
