...
int main()
{
	ifstream ifile(std::string(DATAPATH)+"1map.dta");
	ofstream ofile(std::string(DATAPATH)+"1RANDmap.dta");
	vector<vector<int>> dataset;
	vector<int> ent(6,0);
	int i = 0;
	cout << RAND_MAX << endl;
	
	while((ifile >> ent[0] >> ent[1] >> ent[2] >> ent[3] >> ent[4]>>ent[5]))
	{
		i++;
