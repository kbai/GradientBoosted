...
#ifdef MPIU
    for(int u = 0; u < n_hidden; u++) MPI_Bcast(W[u],5*n_v,MPI_DOUBLE, 0,MPI_COMM_WORLD);
...
#else
			dW[i][5*j + ns] += lr * (ph_mean[i] * input[5*jr + ns] - nh_means[i] * nv_means[5*jr+ns]) / N;
#endif
...
#else
    dh[i] += lr * (ph_mean[i] - nh_means[i]) / N;
#endif
...
#else
		dv[5*i + ns] += lr * (input[5*ir + ns] - nv_means[5 * ir + ns]) / N;
#endif
...
void RBM:: mpigath()
{
	...
	for(int  i = 0; i < n_hidden; i++)
	{
		MPI_Allreduce(dW[i],sW[i],5*n_visible,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
	}
  MPI_Allreduce(dh,sh,n_hidden,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
  MPI_Allreduce(dv,sv,5*n_visible,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
...
void test_rbm() {
	...
	int k = 3;
	vector<int> testuid, trainuid;
	vector<vector<int>> trainset,testset,train_index, test_index;
	load_all_data(trainset, train_index,  trainuid, testset, test_index, testuid);
...
//	int len = trainset.size();
	...
	for(int iepo = 0 ; iepo < 30; iepo++)
	{
		for(int y = 0 ; y < len; y++)
		{
			...
			int iy;
#ifdef MPIU
			iy = (y + (myrank*len/nproc))%len;
#else
			iy = y;
#endif
			rbm.contrastive_divergence(trainset[iy], train_index[iy], learning_rate, k);
			...
			if(y%10 == 0)
			{
			 ...
			 if(y%10000 == 0) cout <<"error:"<<compute_loss(trainuid,testuid,trainset,train_index,testset,test_index,rbm) << endl;
