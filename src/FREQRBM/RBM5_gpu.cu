  ...
  for(int ir=0; 10 * ir<imovie.size(); ir++) {
	...
	int iu  = imovie[10 * ir + 1];
	  ...
	  for(int ns = 0; ns < 5; ns++)
	  {
		...
		vbu[5*iu + ns]			=  2.5*lr*(bias - 0.001* bu[5*iu+ns]) + momentum * vbu[5*iu + ns];
		bu[5*iu + ns]           += vbu[5*iu+ns];
		vbut[5*iut+ns]			=  8.0 *lr*(bias - 0.001* but[5*iut+ns]) + momentum * vbut[5*iut+ns];
		but[5*iut+ns]			+=	vbut[5*iut+ns];
		vbif[5*((30*im)+ife)+ns]=  0.2*lr*(bias - 0.001* bif[5*((30*im)+ife)+ns]) + momentum * vbif[5*((30*im)+ife)+ns]; 
		bif[5*((30*im)+ife)+ns] += vbif[5*((30*im)+ife)+ns];
#else
		vbias[5*im + ns]        +=  lr * (bias - 0.001 *vbias[5*im + ns]);
		bu[5*iu + ns]			+=  2.5*lr*(bias - 0.001* bu[5*iu+ns]);
		but[5*iut+ns]			+=  8.0*lr*(bias - 0.001* but[5*iut+ns]);
		bif[5*((30*im)+ife)+ns] +=  0.2*lr*(bias - 0.001* bif[5*((30*im)+ife)+ns]);
#endif
	  }
  }
  mtx.unlock();
...
void RBM::sample_v_given_h(const int *h0_sample,const vector<int>& imovie,double *mean, int *sample) const
{
  for(int ir=0; 10 * ir<imovie.size(); ir++) {
	...
	for(int ns = 0; ns < 5; ns++)
	{
		propdown(h0_sample, im, vbias + 5 * im, bif + 5*ife, but + 5* iut, bu+5*iu,mean + 5*ir);
