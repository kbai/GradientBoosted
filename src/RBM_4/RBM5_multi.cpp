...
void RBM::output() 
{
  cout << "W:"<<W[e()%n_hidden][e()%n_visible] <<"\t hbias:" << hbias[e()%n_hidden] <<"\t vbias:" << 
	  vbias[e()%(5*n_visible)] <<"\t bu:" <<bu[e()%(5*NUSER)] <<"\t buf:"<<but[e()%(5*NLARGE)]
	  <<"\t  bif:"<<bif[e()%(5*NMOVIE*30)] << endl;


}

RBM::RBM(int size, int n_v, int n_h, double **w, double *hb, double *vb, vector<vector<float>> &c) {
  N = size;
  n_visible = n_v;
  n_hidden = n_h;
	...
	{
//		if(c[i%5][i/5] == 0) cout << i%5 <<"\t" << i/5 << endl;
	vbias[i] = 0.0;
		//vbias[i] = log((c[i%5][i/5] + 1.0)/(c[0][i/5] + c[1][i/5]+c[2][i/5] + c[3][i/5]+c[4][i/5]+5.0));
	}
...
#ifdef MM
			vW[i][5*j+ns] = lr * (ph_mean[i] * input[5*jr + ns] - nh_means[i] * nv_means[5*jr+ns] - LAMBDA * W[i][5*j+ns]) + momentum* vW[i][5*j+ns];
			W[i][5*j+ns] += vW[i][5*j+ns];
#else
			W[i][5*j + ns] += 0.5 * lr * (ph_mean[i] * input[5*jr + ns] - nh_means[i] * nv_means[5*jr+ns] - LAMBDA * W[i][5*j+ns]);
#endif
...
#ifdef MM
    vhbias[i] = lr * (ph_mean[i] - nh_means[i] - LAMBDA * hbias[i]) + momentum * vhbias[i];
    hbias[i] += vhbias[i]; 
#else
	hbias[i] += 0.25 * lr * (ph_mean[i] - nh_means[i] - LAMBDA * hbias[i]); 
#endif	
  }
  //cout << "cd2" << endl;
...
#ifdef MM
		vvbias[5*im + ns]       =  lr * (bias - LAMBDA*vbias[5*im + ns]) + momentum*vvbias[5*im+ns];
		vbias[5*im + ns]		+= vvbias[5*im + ns];
		vbu[5*iu + ns]			=  0.25*lr*(bias - LAMBDA* bu[5*iu+ns]) + momentum * vbu[5*iu + ns];
		bu[5*iu + ns]           += vbu[5*iu+ns];
		vbut[5*iut+ns]			=  0.80 *lr*(bias - LAMBDA* but[5*iut+ns]) + momentum * vbut[5*iut+ns];
		but[5*iut+ns]			+=	vbut[5*iut+ns];
		vbif[5*((30*im)+ife)+ns]=  0.02*lr*(bias - LAMBDA* bif[5*((30*im)+ife)+ns]) + momentum * vbif[5*((30*im)+ife)+ns]; 
		bif[5*((30*im)+ife)+ns] += vbif[5*((30*im)+ife)+ns];
#else
		vbias[5*im + ns]        += 4.0 * lr * (bias - LAMBDA *vbias[5*im + ns]);
		bu[5*iu + ns]			+= 0* 0.25*lr*(bias - LAMBDA* bu[5*iu+ns]);
		but[5*iut+ns]			+= 0* 0.80*lr*(bias - LAMBDA* but[5*iut+ns]);
		bif[5*((30*im)+ife)+ns] += 0* 0.02*lr*(bias - LAMBDA* bif[5*((30*im)+ife)+ns]);
#endif
	  }
  }
  mtx.unlock();
...
#define LOADDATA(A,B,C,D)\
    ...
    {\
		...
		b.push_back(tmp[4]);\
		if(tmp[5]!=0) biasinit[tmp[5]-1][tmp[1]-1] += 1.0;\
...
void test_rbm(string& f1, string& f2) {
	...
	vector<vector<float>> biasinit(5,vector<float>(NMOVIE,0));
	vector<vector<int>> trainset,testset,train_index, test_index;
	vector<vector<int>> quizset, quiz_index;
	load_all_data(trainset, train_index,  trainuid, testset, test_index, testuid, quizset, quiz_index, quizuid, biasinit);
	int train_N = trainset.size();
	double learning_rate = 0.01;

	RBM rbm(train_N,  n_visible, n_hidden, NULL, NULL, NULL, biasinit);
	RBM rbm2(rbm);
	cout << "1111" << endl;
	int iu = 0;
...
//    	 cout <<"error" << compute_loss(trainuid,testuid,trainset,train_index,testset,test_index,rbm) << endl;
	...
	for(int iepo = 0 ; iepo < 50; iepo++)
	{
		...
		if(y%10000 == 0)
		{
			...
			{
				int iy = (y+it)%len;
				threads[it] = std::thread(&RBM::contrastive_divergence,&rbm,std::ref(trainset[order[iy]]),std::ref(train_index[order[iy]]), learning_rate,0.9 ,k);
			}
		...
		//cout <<"4545" <<endl;
		 ...
		 if(writedata[0].joinable()) writedata[0].join();
		 if(writedata[1].joinable()) writedata[1].join();
		 writedata[0] = std::thread(compute_loss,std::ref(trainuid),std::ref(trainuid),std::ref(trainset),std::ref(train_index),std::ref(trainset),std::ref(train_index),std::ref(rbm2),"residue50.txt");
		 if(writedata[0].joinable()) writedata[0].join();
