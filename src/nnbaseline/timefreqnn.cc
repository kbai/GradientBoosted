...
class nn
{
	...
	nn(int _neuron, int _pred):
		...
		w2(_neuron, 0),
		b1(_neuron,0),
		bu(NUSER,vector<float>(_neuron,0)),
		bm(NMOVIE,vector<float>(_neuron,0)),
		bt(NTIME,vector<float>(_neuron,0)),
		bif(NMOVIE, vector<vector<float>>(30, vector<float>(_neuron,0)))
	{
		n_neuron = _neuron;
		n_pred = _pred;
...
//		cout << "bbbbbbbbbb"<< endl;
		...
		for(int i = 0 ; i < n_neuron; i++)
		{
			...
			b1[i] +=  con2;
			bm[im][i] += 0.1 * (con2 - bm[im][i] * lr * 0.1);
			bu[iu][i] += 0.5 * (con2 - bu[iu][i] * lr * 0.1);
			bif[im][ife][i] += 0.5 * (con2 - bif[im][ife][i] * lr * 0.1);
...
//	vector<float> w2(n_neuron,0);
	...
	ifstream a("../../data/1.dta");
	ifstream b("../../data/4.dta");
	ifstream c("../../data/5.dta");
...
//			blender.sgd(trainset[j], trainlabel[j]);
	...
	//		threads[k-j] = std::thread(&nn::sgd, &blender,std::ref(trainset[order[k]]),trainlabel[order[k]] );
		...
		if(i%20 == 1)
		{
			ofstream c(std::string(argv[1])+std::to_string(i)+"probe.txt");
			for(int p = 0; p < testset.size(); p++)
				c<< blender.predict(testset[p], testiu[p], testim[p],testit[p],testife[p]) << endl;
			c.close();

			ofstream d(std::string(argv[1])+std::to_string(i)+"quiz.txt");
