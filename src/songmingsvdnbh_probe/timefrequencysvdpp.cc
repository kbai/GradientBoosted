...
void load_all_data(vector<Feature> &v1, vector<Feature> &v2, vector<Feature> &v3, vector<Feature> &v4, vector<Feature> &v5, vector<float> &Tu, vector<int> &Fut, vector<vector<Feature>> &Ru, vector<int>& Rm, vector<vector<Feature>> &Nu){
...
public:
    TimeFrequencySVD(vector<float> &Tuu, vector<int> &Futt, vector<vector<Feature>> &Ruu,vector<int>&Rmm, vector<vector<Feature>> &Nuu) : Tu(Tuu), Fut(Futt), Ru(Ruu), Rm(Rmm),Nu(Nuu) {
...
//	vector<vector<int>> Fut(NUSER, vector<int>(NTIME));
	...
	TimeFrequencySVD model(Tu, Fut, Ru, Rm,  Nu);
	...
	for(int i = 0; i < 200; i++){
		...
		if(testscore  < mintestscore){
			...
			model.predict(quiz, "quiz1000_"+ to_string(i) +".dta");
		}
	}
}
