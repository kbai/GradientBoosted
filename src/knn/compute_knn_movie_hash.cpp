...
int main()
{
	...
	ifstream neighbour("../../data/1svd200correlation.txt");
	ifstream testsetf("../../data/4map.dta");
	ifstream testr("../../data/residue4_200.txt");
	ofstream output("knntrainsvd200_may27.txt");
	float rmse = 0.0;
	int ttct = 0;
	float rmse1 = 0.0;
...
//		tmp.irat -= t[5];
	...
	for(int i = 0; i < NMOVIE; i++)
	{
		int id;
		neighbour >> id;
//		cout <<"hahahaha:" <<id <<"\t" << i <<  endl;
		assert(id == i+1);
		for(int j = 0; j < UNB; j++)
		{
			if(j < UNB) neighbour >> cot[i][j].imov >> cot[i][j].corr >> cot[i][j].count;
//			if(j > 4800 && j < 4820) cout<<j<<"\t" << cot[i][j].imov<<"\t" << cot[i][j].corr<<"\t"<<cot[i][j].count  << endl;
	...
	while(!infile.eof())
	{
		...
		if(u[0] > curr_user)
		{
			...
			while(testset[iii].iuse == curr_user)
			{
					...
					for(auto & a: inter)
					{
						float tf = (1./(1.0+ 0.004*(abs(a.itim - testset[iii].itim))));
