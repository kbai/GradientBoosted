...
int main()
{
	...
	ifstream neighbour("../../data/movie_neighbour_svd200_neib5000.txt");
	ifstream testsetf("../../data/4map.dta");
	ifstream testr("../../data/residue4_200.txt");
	ofstream output("knnpredict_svd200_neib30.txt");
	float rmse = 0.0;
	int ttct = 0;
	float rmse1 = 0.0;
	...
	while(!infile.eof())
	{
		...
		if(u[0] > curr_user)
		{
			...
			while(testset[iii].iuse == curr_user)
			{
					...
					for(auto & a: inter)
					{
						float tf = (1./(1.0+ 0.00*(abs(a.itim - testset[iii].itim))));
