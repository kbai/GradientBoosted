...
//			cout <<f1.compute_RMSE(abk) << endl;
			...
			f1.compute_residue(abk,std::string("residue1_200.txt"));
			f2.compute_residue(abk,std::string("residue4_200.txt"));
			return;
...
int main(int argc, char** argv)
{
	...
	ifstream infile(std::string(DATAPATH)+"1map.dta");
...
//
		...
		if(i%20 == 0)
		{
			if(th1.joinable()) th1.join();
			abk.retrieve_gpu();
			th1 = std::thread(output,std::ref(outfilename),std::ref(abk),std::ref(fset),std::ref(tset2),std::ref(tset3));
		}
		end = clock();
	...
	fset.compute_residue(abk,std::string("residue1_200.txt"));
	tset2.compute_residue(abk,std::string("residue4_200.txt"));
}
