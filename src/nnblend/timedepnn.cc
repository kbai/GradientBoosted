...
class nn
{
	...
	float predict(vector<float> & data,int iu, int im, int it)
	{
		...
		for(int i = 0; i < n_neuron; i++)
		{
			...
			g1[i] = sigmoid(f1[i]+b1[i] + bm[im][i] + bu[iu][i] + bt[it][i]);
		}
		for(int i = 0; i < n_neuron; i++)
		{
			f2 += g1[i] * w2[i];
	...
	float sgd(vector<float>& data,int iu, int im,int it, float label)
	{
		vector<float> f1(n_neuron,0.0);
		vector<float> g1(n_neuron,0.0);
		float f2 = 0.0;
...
//		insamplerror = sqrt(0.999*insamplerror*insamplerror + 0.001 *(error*error));
		...
		for(int i = 0 ; i < n_neuron; i++)
		{
			...
			b1[i] +=  con2;
			bm[im][i] += 0.5 * (con2 - bm[im][i] * lr * 0.01);
	...
	double compute_rmse(vector<vector<float>>& dataset,vector<int>&iu, vector<int>&im,vector<int>&it,  vector<float> &label)
	{
		double error = 0.0;
		int n = dataset.size();
		for(int i = 0; i < dataset.size(); i++)
		{
			error += pow((predict(dataset[i],iu[i],im[i], it[i]) - label[i]),2.0)/n; 
		}
		return sqrt(error);
	}
...
void load_data_set(ifstream& a,int n, vector<vector<float>>&x,vector<int>&iu, vector<int>&im,vector<int>&it, vector<float>&y)
{
	vector<float> tmp(n,0);;
	float tmp2;
	float tu,tm,tt,junk2;
	int p = 0;
	while(!(a.eof()))
	{
...
void load_quiz_set(ifstream& c,int n, vector<vector<float>>&x,vector<int>&iu, vector<int>&im, vector<int>&it)
{
	vector<float> tmp(n,0);;
	int a = 0;
	float tu,tm, tt, junk2;
...
//	vector<float> w2(n_neuron,0);
	...
	load_data_set(a, n_pred, trainset, trainiu, trainim,trainit, trainlabel);
	load_data_set(b, n_pred, testset,  testiu, testim, testit,testlabel);
	load_quiz_set(c, n_pred, quizset, quiziu, quizim,quizit); 
...
//			blender.sgd(trainset[j], trainlabel[j]);
			...
			{
				if(k < trainset.size())
					blender.sgd(trainset[k], trainiu[k], trainim[k],trainit[k], trainlabel[k]);
