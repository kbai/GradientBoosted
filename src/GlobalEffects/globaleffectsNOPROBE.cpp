...
int main(int argc, char **argv) {
     ...
     std::ifstream infile("../../data/1bethere.dta");
    ...
    printf("asdfasdf\n");
     ...
     std::ofstream myfile3 ("../../data/num_movie_ratings.dta");


     std::ofstream myfile4 ("../../data/num_user_ratings.dta");

     for(int i = 1; i < global_user.size(); i++)
     {   
        myfile << global_user[i] << endl;          
     }

     for(int i = 1; i < global_movie.size(); i++)
     {   
        myfile2 << global_movie[i] << endl;          
     }

     for(int i = 1; i < num_movie.size(); i++)
     {   
        myfile3 << num_movie[i] << endl;          
     }


     for(int i = 1; i < num_user.size(); i++)
     {   
